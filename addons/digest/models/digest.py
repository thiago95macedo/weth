...
class Digest(models.Model):
    ...
    def compute_preferences(self, company, user):
        ...
        if self._context.get('digest_slowdown'):
            ...
            preferences.append('<p>%s<br /><a href="/digest/%s/set_periodicity?periodicity=weekly" target="_blank" style="color:#003C7D; font-weight: bold;">%s</a></p>' % (
                _('Prefer a broader overview ?'),
                self.id,
                _('Switch to weekly Digests')
            ))
        if user.has_group('base.group_erp_manager'):
            preferences.append('<p>%s<br /><a href="/web#view_type=form&amp;model=%s&amp;id=%s" target="_blank" style="color:#003C7D; font-weight: bold;">%s</a></p>' % (
