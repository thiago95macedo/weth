...
class TestOdoobot(TestMailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.odoobot_ping_body = '<a href="http://odoo.com/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@WethBot</a>' % (cls.odoobot.id, cls.odoobot.id)
