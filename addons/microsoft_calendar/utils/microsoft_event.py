...
class MicrosoftEvent(abc.Set):
    ...
    Inspired by WETH recordset, one instance can be a single Microsoft event or a
    ...
    def _meta_odoo_id(self, microsoft_guid):
        """Returns the WETH id stored in the Microsoft Event metadata.
    ...
    def odoo_ids(self):
        """
        Get the list of WETH event ids already mapped with Outlook events (self)
        """
        return tuple(e._odoo_id for e in self if e._odoo_id)

    def _load_odoo_ids_from_db(self, env, force_model=None):
        """
        Map Microsoft events to existing WETH events:
        1) extract unmapped events only,
        2) match WETH events and Outlook events which have both a ICalUId set,
        ...
        # 1. try to match unmapped events with WETH events using their iCalUId
        ...
        # 2. try to match unmapped events with WETH events using their id
        unmapped_events = self.filter(lambda e: e.id not in mapped_events)
        ...
        for ms_event in unmapped_events:
            ...
            if odoo_event:
                ...
                # don't forget to also set the global event ID on the WETH event to ease
    ...
    def owner_id(self, env):
        ...
        1) the current WETH user is the organizer of the event according to Outlook event, so return his id.
        2) the current WETH user is NOT the organizer and:
           2.1) we are able to find a WETH user using the Outlook event organizer email address and we use his id,
           2.2) we are NOT able to find a WETH user matching the organizer email address and we return False, meaning
                that no WETH user will be able to modify this event. All modifications will be done from Outlook.
        """
        if self.isOrganizer:
            return env.user.id
        if self.organizer.get('emailAddress') and self.organizer.get('emailAddress').get('address'):
            # Warning: In Microsoft: 1 email = 1 user; but in WETH several users might have the same email
    ...
    def match_with_odoo_events(self, env) -> 'MicrosoftEvent':
        """
        Match Outlook events (self) with existing WETH events, and return the list of matched events
        """
